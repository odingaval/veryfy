...
Update: `issue_license` now accepts the holder wallet explicitly:

```txt
issue_license(asset_hash, holder, expiry)
```
...
Important implication: the contract does not store `licenseNumber`, `holderName`, `licenseType`, or `issuerWallet` as separate license fields. Veryfy gives meaning to `asset_hash` by hashing those off-chain fields deterministically. The holder wallet is stored on-chain because it is needed for ownership/verification.
